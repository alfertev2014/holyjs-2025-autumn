...
1. Это и мощная система типов, за которой стоят хитрые математические идеи. И в них тоже бывает полезно погрузиться. Здесь я сразу порекомендую книгу Бенжамина Пирса "Типы в языках программирования", являющуются хорошей входной точкой в тему.
...
1. В основном, будем касаться вопросов надёжности системы типов TypeScript
2. Посмотрим на отношение подтипов как на фундамент системы типов, как оно должно выглядеть в теории
3. А потом посмотрим, как с этим в TypeScript. Многие языки, имея под собой внушительную теорию, вынуждены на практике идти на компримисы при реализации. И вслучае с TypeScript расхождение реальной реализации с теоретическими идеями больше, чем кажется. Посмотрим, в чём отличие проверки совместимости типов в TypeScript от настоящего отношения подтипов.

4. И всё это может приводить к ошибкам в runtime, как говорится, "на ровном месте", когда код выглядит чистеньким и красивым, ошибок компиляции нет, никаких хитрых приёмов с типами не применяется, все типы простые, а мы получаем ошибку в runtime. Увидим это на примерах.
-->
---
layout: default
---
...
При дальнейшем его освоении разработчики начинают больше задумываться о вопросах его надёжности, но не все изучают его достаточно глубоко.

А разработчики, пришедние с других, возможно, более строгих языков, пытаются рассуждать о TypeScript по аналогии с другими языками и тоже до конца не изучают его.
