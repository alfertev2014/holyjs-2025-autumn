...
1. Это и мощная система типов, за которой стоят хитрые математические основы. И в них тоже бывает полезно погрузиться. Здесь я сразу порекомендую книгу Бенжамина Пирса "Типы в языках программирования", где даётся хорошее введение в теорию типов.

2. И система типов TypeScript достаточно мудрёная, что можно на типах даже программировать, строить сложные утилитные типы или даже использовать тайпчекер как интерпретатор.
