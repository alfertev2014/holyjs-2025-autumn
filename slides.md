...
<ul>
<li><b v-mark.red="2">Фанат систем типов</b></li>
...
3. А затем посмотрим, в чём отличие проверки совместимости типов в TypeScript от настоящего математически строгого отношения подтипов.
...
<!--
Ничего хардкорного рассказывать не буду. Полезно будет послушать как для начинающих, так и для разработчиков с некоторым опытом.

TypeScript имеет низкий порог входа на первых шагах. Но не все дальше изучают его достаточно глубоко.

А разработчики, пришедние с других, возможно, более строгих языков, пытаются рассуждать о TypeScript по аналогии со своим опытом и тоже не сразу изучают его нюансы. Как я, например.
...
---
layout: image
image: ./images/first_time.png
dragPos:
  cpp: 408,2,148,_
---

<img v-drag="'cpp'" src="./images/cpp_logo.svg" />
...
- Нужно придерживаться best practices
- Ненадёжность можно и нужно контроллировать
- Есть проблемы, за которыми проследить сложнее

</v-clicks>

<!--
Повторим ещё раз:
1 - Система типов TypeScript ненадёжна, и так и было задумано
2 - И для достижения корректности программ нужно придерживаться определённых практик
3 - Есть причины ненадёжности, которые можно и нужно держать под контролем
4 - Есть проблемы в языке, которые незаметно создают риск для всего кода
-->
---
layout: section
---
...
<!--
И многие из этих проблем связаны с отношением подтипов, потому я считаю, что именно его стоит рассмотреть подробнее.
-->
---
layout: default
---
...
<br />
<div class="text-center" style="font-size: 3rem">
<b><span v-click="2">B &lt;: A </span><span v-click="3">?</span></b>
</div>
...
<!--
Отношение подтипов как отношение порядка должно быть рефлексивным, транзитивным и антисимметричным.
-->
---
layout: section
---
...
В него можно записать что угодно.

</div>
<div v-click="1">

Его можно записать куда угодно
...
<div v-click class="reference">
<p><b>Excess Property Checks</b></p>
<p><a style="font-size: 0.9rem" href="https://www.typescriptlang.org/docs/handbook/2/objects.html#excess-property-checks">https://www.typescriptlang.org/docs/handbook/2/objects.html#excess-property-checks</a></p>
</div>
...
**Не является отношением частичного порядка**
...
- Опциональные аргументы и properties позволяют обойти ограничение совместимости типов
...
- Изучайте свой основной язык, на котором пишете
