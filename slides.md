...
<!--
Сегодня я хочу поговорить немного о другом, на более узкую тему, а именно, про некоторые проблемы системы типов TypeScript, о которых не часто вспоминают, но это базовые знания о языке. Многие языки, имея под собой внушительную теорию, вынуждены на практике идти на компримисы при реализации. И вслучае с TypeScript расхождение реальной реализации с теоретическими идеями больше, чем кажется. И это может приводить к ошибкам, как говорится, "на ровном месте", когда код выглядит чистеньким и красивым, ошибок компиляции нет, и даже никаких хитрых приёмов с типами не применяется, все типы простые, а мы получаем ошибку в runtime.
-->
---
layout: default
---
...
<!--
Сам TypeScript имеет низкий порог входа на первых шагах - просто добавляем немного типов в JavaScript. При дальнейшем его освоении разработчики начинают больше задумываться о вопросах его надёжности, но не все изучают его достаточно глубоко. А от "мастеров" в докладах на конференциях и в статьях в интернете больше встречается либо TypeScript-хардкор, либо TypeScript-хэйт, отказ от типизации. Отчего может сложиться неправильное представление о языке, его возможностях и ограничениях.
-->
---
layout: default
---
...
3. Отношение подтипов и TypeScript
4. Проблемы подтипов в TypeScript на примерах
5. Заключение

<!--
 Разберём, что такое отношение подтипов и какие с ним связаны проблемы, на примерах. И в конце подумаем, что с этим можно сделать.
-->
---
layout: section
---

<h1><span class="text-brand-accent-color">1. </span>Проблема надёжности типов</h1>
...
<h1><span class="text-brand-accent-color">2. </span>Ненадёжность системы типов TypeScript</h1>
...
<h1><span class="text-brand-accent-color">3. </span>Отношение подтипов в TypeScript</h1>
...
<h1><span class="text-brand-accent-color">4. </span>Проблемы подтипов в TypeScript на примерах</h1>
...
<h1><span class="text-brand-accent-color">5. </span>Заключение</h1>
